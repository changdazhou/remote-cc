...
- **终端渲染**：改用 WebGL 渲染，选区与文字严格对齐、不再遮住首个字符；网页字体加载完成或终端切回可见时自动重新测量字宽；不支持 WebGL 时自动回退
- **滚动体验**：上划查看历史时不再被新输出拉回，10 秒无操作后自动回到底部；新增「回到底部 / 新输出」按钮；去掉滚轮动画，手机滑动支持惯性
- **终端配色**：Agent 查询终端前景/背景色（OSC 10/11 等）时正常应答，按当前主题配色；回放历史或多设备同时连接时不会重复应答
- **上传**：实时显示进度、速度和剩余时间，可取消；单文件上限 10 GB，磁盘空间不足时提前提示
- **Agent 命令**：状态中始终显示解析后的完整可执行文件路径；支持在本机 `~/.rcc/agent.env` 中配置首选命令（`RCC_<AGENT>_PREFERRED`）和 `CODEX_HOME`

### 2026-09-23

- **大文件上传/下载不再卡死**：上传和下载改为服务端流式读写、浏览器直接发送文件，不再整体读入内存；上传实时显示进度、速度和剩余时间，可随时取消，单文件上限 10 GB；失败会给出原因；中文文件名下载修复
- **启动参数透传**：新建/恢复会话可临时填写 `--model xxx` 等参数，原样传给 Agent CLI（Web 与 `rcc-tui` 均支持）
- **Grok 支持**：新增 Grok Agent，支持新建会话、历史恢复和自定义命令（`GROK_BIN` / `GROK_PROXY`）
- **会话与错误提示**：同一目录可以同时开多个新会话；Agent 启动失败（目录不存在、会话数上限等）会直接显示在终端里，HTTP 回退不再无限重试；Agent 会话日志按大小裁剪，长时间高输出不再拖慢服务
- **易用性问题集中修复**：修正会话重命名（Esc 误保存、Enter 双触发、不自动聚焦）、恢复历史列表不刷新、文件浏览器目录单击进入与预览竞态、上传部分失败无提示、无扩展名文本文件误判二进制与 UTF-8 截断乱码等问题
- **连接稳定性**：WebSocket 重连期间输入不再被静默丢弃（缓冲后重放）；共享终端在 HTTP 回退后会周期性重试升级回 WebSocket；设置页重连延迟/保活开关现已实际生效
- **移动端与设置**：修复复制模式退出后输出假死、Android 回车不发送、scrollback 调整不实时；修改密码新增 ≥8 位校验
